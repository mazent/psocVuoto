...
static void esamina_blocco(uint8_t * set_lib, BLK_T blk)
{
...
//        DBG_PRINTF("blocco %d", blk) ;
        ...
        // Tutto usato
        BIT_zeri(set_lib, FTL_FSECT_NUM) ;
...
//              DBG_PRINTF("\t %d: usato", fis) ;
                ...
                if ( uguale_a( pF, FTL_ERASED, sizeof(FTL_SECT_F) ) ) {
                    // Libero!
...
static void esamina_blocco_cancellato(uint8_t * set_lib, BLK_T blk)
{
    FTL_SECT_F * pF = soc_malloc( sizeof(FTL_SECT_F) ) ;
    if (NULL != pF) {
        size_t ofs = 0 ;
        ...
        for ( size_t fis = 0 ; fis < FTL_FSECT_NUM ;
            ...
            else if ( uguale_a( pF, FTL_ERASED, sizeof(FTL_SECT_F) ) ) {
                // Libero!
                BIT_uno(set_lib, fis) ;
            }
            else if (LRU_NONVALE == pF->lru ) {
...
static bool elimina_fis(BLK_T blk, SECT_T fis)
{
    ...
    CHECK( esito = pOp->pfWrite( blk, ofs, &nv, sizeof(LRU_T) ) ) ;
