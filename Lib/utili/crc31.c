/*
 * Polinomio: 0x31 (x^8 + x^5 + x^4 + 1)
 * Reflect input: False
 * Reflect output: False
 * Final XOR: 0x00
 *
 * vedi http://www.sunshine2k.de/coding/javascript/crc/crc_js.html
 */

#include <stdint.h>

static const uint8_t tabella[] = {
    ,
} ;

static uint8_t aggiorna(uint8_t val, uint8_t crc)
{
    uint8_t tmp ;

    tmp = (uint8_t)( (crc >> 8) ^ val ) ;
    crc = (uint8_t)( (crc << 8) ^ tabella[ tmp ] ) ;

    return crc ;
}

uint8_t CRC_31_v(uint8_t crc, const void * v, int dim)
{
    const uint8_t * vett = v ;

    for (int i = 0 ; i < dim ; i++) {
        uint8_t val = vett[i] ;

        crc = aggiorna(val, crc) ;
    }

    return crc ;
}

uint8_t CRC_31_b(uint8_t crc, uint8_t val)
{
    return aggiorna(val, crc) ;
}
